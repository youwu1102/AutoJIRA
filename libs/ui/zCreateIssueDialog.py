...
import yIssueFromDialog
from os.path import exists
from libs import Utility, GlobalVariable, JIRA
import webbrowser
...
class CreateIssueDialog(wx.Dialog):
    ...
    def on_create(self, event):
        ...
        dict_issue = yIssueFromDialog.generate(self)
        state, response, error = JIRA.create(data=dict_issue, account='qrd_automation', password='1234Abcd')
        if state == 0:
            response = eval(response)
            self.__show_result(response.get('key'))
            self.Destroy()
        else:
            print 'ErrorInCreateIssueDialog'
